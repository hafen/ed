...
#    qqCore(sort(resid(x)), x$parms$k, length(x$dat$x), normal=normal, dat=dat, ...)
# }
# 
# qq.edPre <- function(x, normal=FALSE, dat=FALSE, ...) {
#    if(is.null(x$f)) {
#       warning("cannot call qq for preliminary estimates if f is not specified")
#       return(NULL)
#    } else {
#       qqCore(sort(x$data$pre - log(x$f(x$data$x))), x$k, length(x$data$x), normal=normal, dat=dat, ...)
#    }
# }
# 
# qqCore <- function(x, k, n, c=1, normal=FALSE, dat=FALSE, ...) {
